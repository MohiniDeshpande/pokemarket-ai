...
import { promptAndInstallSkills } from "./installSkills.js";
...
const validateNodeVersion = () => {
    ...
    if (!nodeVersion || !semver.satisfies(nodeVersion, requiredNodeRange)) {
        console.log(chalk.red(`Node version satisfying "${requiredNodeRange}" is required for the MongoDB Local MCP Server. Current version: ${nodeVersion ?? "unknown"}. Please install or activate a compatible version.`));
        printNewLine();
        return false;
    }
    return true;
...
const validateCredentials = (connectionString, serviceAccountId, serviceAccountSecret, hasDocker) => {
    ...
    if (!connectionString && (!serviceAccountId || !serviceAccountSecret)) {
        console.log(chalk.yellow("No credentials have been provided, so the MCP Server will not be able to access your MongoDB data or Atlas project."));
        if (hasDocker) {
            console.log(chalk.yellow("Since you have Docker running, you can still use the MCP server with a local Atlas instance running in a container."));
        }
        else {
            console.log(chalk.red("Since you don't have Docker running, you can only connect to a MongoDB instance dynamically, " +
                chalk.bold(chalk.red("which is strongly discouraged as it will expose your connection string to the LLM."))));
        }
        printNewLine();
    }
};
...
const formatSkillsResult = (result) => {
...
const guideUserWithSetupSuccess = (displayName, availablePrompts, skillsResult) => {
    printNewLine();
    console.log(chalk.green(`Setup complete! You can now use the MongoDB MCP Server in ${displayName}. You will probably need to restart your application to see the changes.\n`));
 ...
 */
export const runSetup = async (config) => {
